...
-- Create enum types
CREATE TYPE user_role AS ENUM ('ATHLETE', 'MENTOR', 'GUARDIAN', 'ADMIN', 'COLLEGE_REP', 'SPONSOR');
CREATE TYPE athlete_level AS ENUM ('SCHOOL', 'DISTRICT', 'STATE', 'NATIONAL', 'INTERNATIONAL');
CREATE TYPE request_status AS ENUM ('PENDING', 'PENDING_GUARDIAN', 'APPROVED', 'REJECTED', 'COMPLETED', 'CANCELLED');
CREATE TYPE mentorship_mode AS ENUM ('ONLINE', 'OFFLINE', 'GROUP', 'CAREER_GUIDANCE', 'TRIAL_PREP');
CREATE TYPE report_category AS ENUM ('HARASSMENT', 'INAPPROPRIATE_LANGUAGE', 'FRAUD', 'UNSAFE_MEETING', 'PRESSURE', 'DISCRIMINATION', 'MISUSE_CONTENT', 'SAFETY_CONCERN', 'OTHER');
CREATE TYPE report_severity AS ENUM ('NORMAL', 'URGENT', 'EMERGENCY');
CREATE TYPE report_status AS ENUM ('SUBMITTED', 'UNDER_REVIEW', 'ESCALATED', 'RESOLVED', 'DISMISSED');
CREATE TYPE scholarship_status AS ENUM ('SAVED', 'APPLYING', 'SUBMITTED', 'SHORTLISTED', 'APPROVED', 'REJECTED');
CREATE TYPE notification_type AS ENUM ('MENTORSHIP', 'SCHOLARSHIP', 'REPORT', 'VERIFICATION', 'REWARD', 'ADMIN', 'CHAT', 'REMINDER');
CREATE TYPE opportunity_type AS ENUM ('TOURNAMENT', 'TRIAL', 'CAMP', 'GOVERNMENT_SCHEME', 'ACADEMY', 'SCHOLARSHIP');
CREATE TYPE content_type AS ENUM ('VIDEO', 'ARTICLE', 'GUIDE', 'EXERCISE', 'NUTRITION', 'MENTAL_HEALTH');
